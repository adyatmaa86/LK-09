...
<body class="bg-[#f8fafc] text-[#1e293b] dark:bg-slate-900 dark:text-slate-100 min-h-screen flex flex-col transition-colors duration-300">
    ...
    <div class="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
            ...
            <a href="{{ route('buku.create') }}" class="w-fit inline-flex items-center px-4 py-2.5 md:px-6 md:py-3 text-sm md:text-base bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl md:rounded-2xl transition-all duration-300 shadow-lg shadow-indigo-200 transform hover:-translate-y-1">
        ...
        <form action="{{ route('buku.do-search') }}" method="POST" class="bg-white dark:bg-slate-800 p-4 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 mb-8 flex flex-col md:flex-row gap-4 items-center">
            ...
            <div class="flex flex-row gap-2 w-full md:w-auto">
                <div class="relative flex-1 md:w-auto custom-dropdown" data-auto-submit="true">
                    ...
                    <button type="button" class="dropdown-btn w-full flex items-center justify-between gap-3 bg-white dark:bg-slate-800 rounded-2xl py-4 px-6 focus:ring-2 focus:ring-indigo-500 transition-all duration-500 text-slate-700 dark:text-slate-300 cursor-pointer md:min-w-[180px] hover:bg-slate-50 dark:hover:bg-slate-700">
                    ...
                    <div class="dropdown-menu absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 overflow-hidden z-50 hidden opacity-0 translate-y-2 transition-all duration-500">

                        <div class="p-2 space-y-0.5">
                            <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors duration-500 {{ session('buku_category', 'Semua Kategori') == 'Semua Kategori' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400' }}" data-value="Semua Kategori">
                            ...
                            <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors duration-500 {{ session('buku_category') == 'Fiksi' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400' }}" data-value="Fiksi">
                            ...
                            <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors duration-500 {{ session('buku_category') == 'Non-Fiksi' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400' }}" data-value="Non-Fiksi">
                            ...
                            <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors duration-500 {{ session('buku_category') == 'Sains' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400' }}" data-value="Sains">
                            ...
                            <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors duration-500 {{ session('buku_category') == 'Sejarah' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400' }}" data-value="Sejarah">
                ...
                <div class="flex gap-2 shrink-0">
                    <button type="submit" class="inline-flex items-center px-6 py-3 bg-slate-800 text-white font-semibold rounded-2xl hover:bg-slate-900 transition-all duration-500 cursor-pointer">
                        Cari
                    </button>
                                    ...
                                    <div class="p-4 flex flex-col flex-1">
                                        ...
                                        <div class="flex items-center justify-between pt-3 border-t border-slate-50 dark:border-slate-700/50 mt-auto">
                                            <div class="flex gap-1">
                                                <a href="{{ route('buku.edit', $buku) }}" class="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-all duration-500 cursor-pointer">
                                                ...
                                                <form action="{{ route('buku.destroy', $buku) }}" method="POST" class="delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all duration-500 cursor-pointer">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="p-6 flex flex-col flex-1">
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-slate-50 dark:border-slate-700/50">
                            <div class="flex gap-2">
                                <a href="{{ route('buku.edit', $buku) }}" class="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-xl transition-all duration-500 cursor-pointer">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <form action="{{ route('buku.destroy', $buku) }}" method="POST" class="delete-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all duration-500 cursor-pointer">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                        </svg>
                                    </button>
