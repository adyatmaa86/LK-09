                ...
                <form action="{{ route('buku.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8" novalidate>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="col-span-full">
                            <label for="title" class="block text-sm font-bold text-slate-700 mb-2">Judul Buku</label>
                            <input type="text" name="title" id="title" value="{{ old('title') }}" required
                                class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all placeholder:text-slate-400 @error('title') ring-2 ring-rose-500 @enderror"
                        ...
                        <div>
                            ...
                            <input type="text" name="author" id="author" value="{{ old('author') }}" required
                                class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all @error('author') ring-2 ring-rose-500 @enderror"
                        ...
                        <!-- Penerbit -->
                        <div>
                            <label for="publisher" class="block text-sm font-bold text-slate-700 mb-2">Penerbit</label>
                            <input type="text" name="publisher" id="publisher" value="{{ old('publisher') }}" required
                                class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all @error('publisher') ring-2 ring-rose-500 @enderror"
                        ...
                        <!-- Tahun Terbit -->
                        <div>
                            <label for="year" class="block text-sm font-bold text-slate-700 mb-2">Tahun Terbit</label>
                            <input type="number" name="year" id="year" value="{{ old('year') }}" required
                                class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all @error('year') ring-2 ring-rose-500 @enderror"
                        ...
                        <div>
                            ...
                            <div class="relative custom-dropdown">
                                <input type="hidden" name="category" class="dropdown-input" value="{{ old('category') }}" required>
                                <button type="button" class="dropdown-btn flex items-center justify-between gap-3 bg-slate-50 rounded-2xl py-4 px-5 focus:ring-2 focus:ring-indigo-500 transition-all text-slate-700 cursor-pointer w-full @error('category') ring-2 ring-rose-500 @enderror hover:bg-slate-100">
                                    <div class="flex items-center gap-2">
                                        <span class="dropdown-text font-semibold text-sm">{{ old('category') ?: 'Pilih Kategori' }}</span>
                                    </div>
                                ...
                                <div class="dropdown-menu absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 hidden opacity-0 translate-y-2 transition-all duration-300">
                                    <div class="p-2 space-y-0.5">
                                        <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 transition-colors {{ old('category') == 'Fiksi' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600' }}" data-value="Fiksi">
                                        ...
                                        <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 transition-colors {{ old('category') == 'Non-Fiksi' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600' }}" data-value="Non-Fiksi">
                                        ...
                                        <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 transition-colors {{ old('category') == 'Teknologi' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600' }}" data-value="Teknologi">
                                        ...
                                        <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 transition-colors {{ old('category') == 'Sains' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600' }}" data-value="Sains">
                                        ...
                                        <div class="dropdown-item flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer hover:bg-indigo-50 transition-colors {{ old('category') == 'Sejarah' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600' }}" data-value="Sejarah">
                    ...
                    <!-- Deskripsi -->
                    <div>
                        <label for="description" class="block text-sm font-bold text-slate-700 mb-2">Deskripsi / Sinopsis</label>
                        <textarea name="description" id="description" rows="5" required
                            class="w-full px-5 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all resize-none @error('description') ring-2 ring-rose-500 @enderror"
                            placeholder="Tuliskan ringkasan isi buku...">{{ old('description') }}</textarea>
                        @error('description') <p class="mt-2 text-sm text-rose-500">{{ $message }}</p> @enderror
                    </div>

                    <!-- Sampul Buku -->
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Sampul Buku</label>
                        <div class="mt-2 flex justify-center px-6 pt-5 pb-6 border-2 @error('cover') border-rose-500 bg-rose-50/50 @else border-slate-100 @enderror border-dashed rounded-3xl bg-slate-50/50 hover:bg-slate-50 transition-colors group">
                        ...
                        <div id="image-preview-container" class="mt-4 hidden">
                            <p class="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider">Pratinjau Sampul:</p>
                    ...
                    <div class="pt-6">
                        <button type="submit" class="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl transition-all shadow-lg shadow-indigo-100 transform hover:-translate-y-1">
                            Simpan Koleksi Buku
                        </button>
                    </div>
                </form>
